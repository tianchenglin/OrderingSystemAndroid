...
public class GetCashiersServlet extends ActionSupport {
	private static final long serialVersionUID = -7811568044252827351L;
	private HttpServletRequest request;
	private HttpServletResponse response;
	
	public GetCashiersServlet(){
	...
	public void doGet( )	throws  IOException {
		...
		CashierDAO cd;
		List<Cashier> cashiers = null; // 从数据库读取订单记录
		JSONObject jObject = new JSONObject();
		JSONObject jData = new JSONObject();

		try {
			cd = new CashierDAO();
			cashiers = cd.getCasher();
			jData.put("Cashiers", cashiers);
